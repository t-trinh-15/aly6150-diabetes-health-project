...
def clean_brfss(brfss: pd.DataFrame) -> pd.DataFrame:
    ...
    # -----------------------------------------------------------------------
    df = df[df["_STATE"].isin(NEW_ENGLAND_STATES.keys())].copy()
    n_after_state = len(df)
    print(f"  Step 2a — after NE state filter:   {n_after_state:>6,} (removed {n_start - n_after_state:,})")
    ...
    # 3. State name lookup
    # -----------------------------------------------------------------------
    df["state_name"] = df["_STATE"].map(NEW_ENGLAND_STATES)
    ...
    # _PA150R4: 1 = met both aerobic+strengthening; 2/3 = partial/none; 9 = NaN
    df["meets_pa_guideline"] = np.select(
        [df["_PA150R4"] == 1, df["_PA150R4"].isin([2, 3])],
        [1.0, 0.0],
        default=np.nan,
    )
    ...
    df["recent_a1c"] = df["CHKHEMO3"].apply(
        lambda x: 1.0 if 1 <= x <= 76 else (0.0 if x == 88 else np.nan)
    )
    ...
    final_cols = [
        # IDs and weights
        "_STATE", "state_name", "_LLCPWT", "_STSTR", "_PSU",
        # Demographics
    ...
    print(f"  Preventive-care eligible subgroup: {n_eligible:,} (ME + NH only)")
